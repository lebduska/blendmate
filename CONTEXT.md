...
- Debounce/throttle layer implemented for high-frequency events (depsgraph, frame_change) with configurable interval (50-200ms).
...
- High-frequency events (depsgraph_update, frame_change) are now throttled via a shared debounce layer with configurable interval (default 100ms).
