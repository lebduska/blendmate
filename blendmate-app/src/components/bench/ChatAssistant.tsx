/**
 * AI Chat Assistant for Blender modeling
...
function buildSceneContext(
  ...
  let context = `Current Blender Scene: "${sceneData.scene.name}"
Objects: ${objects.length} total (${meshes.length} meshes, ${lights.length} lights, ${cameras.length} cameras)
`;

  if (selectedObjectName && sceneData.objects[selectedObjectName]) {
    const obj = sceneData.objects[selectedObjectName];
    context += `
Selected Object: "${obj.name}" (${obj.type})
  - Location: [${obj.location.map(v => v.toFixed(2)).join(', ')}]
  - Rotation: [${obj.rotation_euler.map(v => (v * 180 / Math.PI).toFixed(1)).join('°, ')}°]
  - Scale: [${obj.scale.map(v => v.toFixed(2)).join(', ')}]
  - Modifiers: ${obj.modifiers.length > 0 ? obj.modifiers.map(m => m.name).join(', ') : 'none'}
`;
    if (obj.mesh) {
      context += `  - Mesh: ${obj.mesh.vertices} verts, ${obj.mesh.polygons} faces\n`;
    }
  } else {
    context += `Selected Object: None\n`;
  }

  // List all objects briefly
  context += `\nAll objects:\n`;
  for (const obj of objects.slice(0, 10)) {
    context += `  - ${obj.name} (${obj.type})${obj.modifiers.length > 0 ? ` [${obj.modifiers.map(m => m.type).join(', ')}]` : ''}\n`;
  }
  if (objects.length > 10) {
    context += `  ... and ${objects.length - 10} more\n`;
  }
...
export default function ChatAssistant({ sceneData, selectedObjectName }: ChatAssistantProps) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.startsWith('OPERATOR ')) {
          ...
          if (spaceIdx > 0) {
            ...
            const valueStr = rest.slice(spaceIdx + 1);
            try {
              const value = JSON.parse(valueStr);
              commands.push({ type: 'property', action: 'set', target, params: { value } });
            } catch {
              commands.push({ type: 'property', action: 'set', target, params: { value: valueStr } });
            }
          }
