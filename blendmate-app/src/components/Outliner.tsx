...
import { ChevronRight, ChevronDown, Folder, Box, Camera, Lightbulb, Speaker, Type, Spline, Circle, Sparkles, Grid3X3, Eye, EyeOff, Move, RotateCcw, Maximize2, Video, VideoOff } from "lucide-react";
...
const OBJECT_TYPE_ICONS: Record<string, React.ReactNode> = {
  MESH: <Box className="size-3.5 text-orange-400" />,
  CAMERA: <Camera className="size-3.5 text-purple-400" />,
  LIGHT: <Lightbulb className="size-3.5 text-yellow-400" />,
  SPEAKER: <Speaker className="size-3.5 text-blue-400" />,
  FONT: <Type className="size-3.5 text-pink-400" />,
  CURVE: <Spline className="size-3.5 text-green-400" />,
  EMPTY: <Circle className="size-3.5 text-gray-400" />,
  ARMATURE: <Sparkles className="size-3.5 text-cyan-400" />,
  LATTICE: <Grid3X3 className="size-3.5 text-indigo-400" />,
};
...
export default function Outliner({
  ...
  const toggleViewportVisibility = useCallback(async (e: React.MouseEvent, objectName: string, currentHidden: boolean) => {
    ...
    if (result.success) {
      console.log(`[Outliner] Toggled viewport visibility for "${objectName}": ${!currentHidden}`);
    } else {
      console.error(`[Outliner] Toggle visibility failed: ${result.error}`);
    }
  ...
  const toggleRenderVisibility = useCallback(async (e: React.MouseEvent, objectName: string, currentHidden: boolean) => {
    ...
    if (result.success) {
      console.log(`[Outliner] Toggled render visibility for "${objectName}": ${!currentHidden}`);
    } else {
      console.error(`[Outliner] Toggle render visibility failed: ${result.error}`);
    }
  }, [observeOutgoing, setProperty]);
  ...
  const commitEdit = useCallback(async (nodeId: string, originalLabel: string) => {
    ...
    if (observeOutgoing && nodeId.startsWith('obj_')) {
      ...
      if (result.success) {
        console.log(`[Outliner] Renamed "${objectName}" to "${result.data?.name}"`);
        // Selection will update when scene refreshes
      } else {
        console.error(`[Outliner] Rename failed: ${result.error}`);
  ...
  const renderNode = (node: TreeNode, depth = 0) => {
    ...
    const icon = node.type === 'collection'
      ...
      : OBJECT_TYPE_ICONS[node.objectType || ''] || <Box className="size-3.5 text-muted-foreground" />;
        ...
        <Tooltip delayDuration={500}>
          <TooltipTrigger asChild>
            <div
              ...
              className={cn(
                "w-full text-left flex items-center gap-2 px-2 py-1.5 rounded-lg transition-all group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/50 cursor-pointer",
                selected
                ...
                <div className="shrink-0 flex items-center gap-0.5">
                  ...
                  >
                    {(node.rawData as BlenderObject).hide_viewport ? (
                      <EyeOff className="size-3" style={{ color: 'var(--islands-color-error)' }} />
                    ) : (
                      <Eye className="size-3" style={{ color: 'var(--islands-color-success)' }} />
                    )}
                  </button>
                  <button
                  ...
                  >
                    {(node.rawData as BlenderObject).hide_render ? (
                      <VideoOff className="size-3" style={{ color: 'var(--islands-color-error)' }} />
                    ) : (
                      <Video className="size-3" style={{ color: 'var(--islands-color-success)' }} />
                    )}
                  </button>
                </div>
              )}
